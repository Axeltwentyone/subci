...
{
    public function __invoke(Request $request, SubscriptionSweeper $sweeper, Availability $availability): JsonResponse
    {
        $user = $request->user();
        $sweeper->run($user);
        ...
        return response()->json([
            ...
            'subscriptions' => SubscriptionResource::collection($user->subscriptions()->with('service')->latest()->get()),
            'payments' => PaymentResource::collection($user->payments()->with('service')->where('status', PaymentStatus::Succeeded)->latest()->limit(100)->get()),
