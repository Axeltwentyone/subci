...
class ReferralTest extends TestCase
{
    ...
    public function test_code_rules(): void
    {
        ...
        $this->postJson('/api/v1/me/referral', ['code' => strtolower($alice->referral_code)])->assertOk()
            ->assertJsonPath('data.referral.referredBy', 'Alice K.')->assertJsonPath('data.referral.feeWaived', true);
    ...
    public function test_referee_pays_no_fee_referrer_gets_credit_used_on_next_payment_host_share_untouched(): void
    {
        $alice = User::factory()->create(['first_name' => 'Alice', 'last_name' => 'K']);
        ...
        // Filleul : frais offerts sur son paiement.
        $payment = $this->payAndAccept($bob);
        $this->assertSame(2400, $payment->amount);
        $this->assertSame(0, $payment->service_fee);
        $this->assertSame(500, $alice->fresh()->referral_credit);
        $this->assertSame('rewarded', Referral::sole()->status);
        $this->actingAs($bob)->getJson('/api/v1/me')->assertJsonPath('data.referral.feeWaived', false);

        // Parrain : 2 400 + 200 de frais − 500 de crédit = 2 100 payés ; l'hôte touche toujours 90 % de 2 400.
        $second = $this->payAndAccept($alice);
        $this->assertSame(2100, $second->amount);
        $this->assertSame(500, $second->credit_used);
        $this->assertSame(0, $alice->fresh()->referral_credit);
        $this->assertSame(2160, Payment::where('source_payment_id', $second->id)->value('amount'));
    }
